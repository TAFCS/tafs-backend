...
const DRY_RUN_XLSX = path.join(
  __dirname,
  '../student-fees-26-27/nnn/campus3_annual_charges_12_installments_dry_run.xlsx',
);

const FEE_TYPE_ID = 4; // Annual Charges
const ACADEMIC_YEAR = '2026-2027';
const CREATED_BY = 'nnn-installments-migration-script';
...
function loadPlan() {
  ...
  const students = summary.filter((r) => r.Status === 'NOT_ISSUED');
...
async function processStudent(plan: ReturnType<typeof loadPlan>[number]) {
  return prisma.$transaction(
    async (tx) => {
      ...
      // NOT by the "Fee Record ID" baked into the dry-run report, which can drift.
...
async function main() {
  ...
  console.log(`Loaded plan for ${plans.length} students (excludes non-NOT_ISSUED students).`);

  if (mode === 'test') {
    const testPlan = plans[0];
    console.log(`Running SINGLE test insertion for CC ${testPlan.studentCC}...`);
    ...
    console.log('Usage: ts-node scripts/nnn-campus3-annual-charges-to-installments.ts <test|all>');
    process.exit(1);
  }
}
