...
function parseArgs(argv: string[]) {
  ...
  const campus_code = String(args['campus-code'] || 'NNZ').trim();
  const gr_prefix = String(args['gr-prefix'] || `${campus_code}-`).trim();
  ...
  return { root, campus_code, gr_prefix, dryRun, type };
}
...
function extractNameFromFilename(filename: string): string | null {
  ...
  if (!base) return null;
  // Remove trailing number and noisy tokens
  return base
...
function normalizeGrPrefix(prefix: string) {
  const p = prefix.trim();
  if (!p) return '';
  return p;
}

function extractNumericLabel(filename: string): number | null {
  const base = path.basename(filename, path.extname(filename));
  const m = base.match(/(\d{1,6})\s*$/);
  if (!m?.[1]) return null;
  const n = Number(m[1]);
...
function deriveGrFromFilename(filename: string, grPrefix: string): string | null {
  const base = path.basename(filename, path.extname(filename));
  // If it already includes something like A-N016 (or NNZ-123), keep it.
  const explicit = base.match(/\b[A-Z]{1,5}-[A-Z]\d{1,6}\b|\b[A-Z]{2,5}-\d{1,6}\b/i);
  if (explicit?.[0]) return explicit[0].toUpperCase();

  const n = extractNumericLabel(filename);
  if (n == null) return null;
  // North format: A-N016 (no dash after N). Always zero-pad to 3 digits for n < 1000.
  const prefix = normalizeGrPrefix(grPrefix);
  const digits = String(n).padStart(3, '0');
  return `${prefix}${digits}`;
}

function grVariants(gr: string): string[] {
  // Match common variants: A-N016, A-N16, A-N0016
  const upper = gr.trim().toUpperCase();
  const m = upper.match(/^([A-Z]{1,5}-[A-Z])0*(\d{1,6})$/);
  if (!m) return [upper];
  const base = m[1];
  const digits = m[2];
  const n = String(Number(digits));
  const pads = [n, n.padStart(2, '0'), n.padStart(3, '0'), n.padStart(4, '0')];
  return Array.from(new Set(pads.map((d) => `${base}${d}`)));
}
...
async function listFilesRecursive(root: string) {
  ...
  const walk = async (dirAbs: string, dirRel: string) => {
    ...
    for (const e of entries) {
      ...
      else if (e.isFile()) out.push({ abs, rel, file: e.name });
    }
  };
  await walk(root, '');
  return out;
...
async function main() {
  const { root, campus_code, gr_prefix, dryRun, type } = parseArgs(process.argv.slice(2));
  if (!root) {
    throw new Error(
      'Usage: ts-node scripts/bulk-upload-north-campus-photos.ts --root "/path/to/NORTH" --campus-code NNZ [--gr-prefix "NNZ-"] [--dry-run] [--type standard|blue_bg]',
    );
  }
  ...
  const parsed = images.map((f) => ({
    ...f,
    gr: deriveGrFromFilename(f.file, gr_prefix),
    name_from_filename: extractNameFromFilename(f.file),
  }));

  const byGr = new Map<string, Array<{ file: string; rel_path: string }>>();
  for (const p of parsed) {
    if (!p.gr) continue;
    const list = byGr.get(p.gr) || [];
    list.push({ file: p.file, rel_path: p.rel });
    byGr.set(p.gr, list);
  }
  ...
  const campus = await prisma.campuses.findFirst({
    where: { campus_code: { equals: campus_code, mode: 'insensitive' } },
    select: { id: true, campus_code: true, campus_name: true },
  });
  if (!campus) {
    throw new Error(`Campus not found for campus_code="${campus_code}". Check campuses table for correct code.`);
  }
  ...
  for (const p of parsed) {
    ...
    if (duplicateGrs.has(gr)) continue;

    try {
      const baseWhere = { deleted_at: null, campus_id: campus.id } as any;
      const variants = grVariants(gr);
      ...
      if (candidates.length === 0 && name_from_filename) {
        ...
        if (nameCandidates.length === 1) {
          ...
          if (!oldGr || oldGr.trim().toUpperCase() !== gr.trim().toUpperCase()) {
            if (!dryRun) {
              await prisma.students.update({ where: { cc: s.cc }, data: { gr_number: gr } });
            }
      ...
      if (dryRun) {
        report.uploaded.push({ file, rel_path: rel, gr, cc: student.cc, full_name: student.full_name, url: '(dry-run)' });
        continue;
      }
      ...
      await prisma.students.update({ where: { cc: student.cc }, data: { [dbField]: url } as any });
  ...
  const outPath = path.join(outDir, `north-campus-photos-report-${nowStamp()}${dryRun ? '-dryrun' : ''}.json`);
