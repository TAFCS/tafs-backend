...
function addMonthsSafe(date: Date, months: number) { const d = new Date(date); const day = d.getUTCDate(); d.setUTCDate(1); d.setUTCMonth(d.getUTCMonth() + months); d.setUTCDate(Math.min(day, new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth() + 1, 0)).getUTCDate())); return d; }
function monthsRemaining(from: Date, until: Date) { return (until.getUTCFullYear() - from.getUTCFullYear()) * 12 + until.getUTCMonth() - from.getUTCMonth() - (until.getUTCDate() > from.getUTCDate() ? 1 : 0); }
...
export class SalaryIncrementsService {
  ...
  private status(employee: any, defaultCycle: number) { const cycle = employee.increment_cycle_months ?? defaultCycle; const anchor = employee.last_increment_at ?? employee.join_date; if (!anchor) return { cycle_months: cycle, anchor_date: null, next_due_date: null, months_remaining: null, status: 'MISSING_ANCHOR' }; const next = addMonthsSafe(dateOnly(anchor), cycle); const left = monthsRemaining(dateOnly(new Date()), next); return { cycle_months: cycle, anchor_date: isoDate(dateOnly(anchor)), next_due_date: isoDate(next), months_remaining: left, status: left <= 0 ? 'DUE' : 'UPCOMING' }; }
  async due(query: DueSalaryIncrementsQueryDto, user: IJwtStaffPayload) { const setting = await this.settings(); const where: Prisma.employee_profilesWhereInput = { ...(user.campusId != null ? { campus_id: user.campusId } : query.campus_id ? { campus_id: query.campus_id } : {}), ...(query.department_id ? { department_id: query.department_id } : {}), ...(query.segment_id ? { segment_id: query.segment_id } : {}), ...(query.class_id ? { employee_class_section_assignments: { some: { class_id: query.class_id } } } : {}), ...(query.search ? { OR: [{ full_name: { contains: query.search, mode: 'insensitive' } }, { employee_code: { contains: query.search, mode: 'insensitive' } }] } : {}) };
    const rows = await this.prisma.employee_profiles.findMany({ where, select: { id: true, full_name: true, employee_code: true, monthly_pay: true, join_date: true, last_increment_at: true, increment_cycle_months: true, employment_status: true, campuses: { select: { campus_name: true } }, departments: { select: { name: true } }, segments: { select: { name: true } } }, orderBy: { full_name: 'asc' } });
    return rows.map(e => ({ employee_id: e.id, name: e.full_name, employee_code: e.employee_code, monthly_pay: e.monthly_pay ? Number(e.monthly_pay) : null, join_date: isoDate(e.join_date), last_increment_at: isoDate(e.last_increment_at), employment_status: e.employment_status, campus: e.campuses?.campus_name ?? null, department: e.departments?.name ?? null, segment: e.segments?.name ?? null, ...this.status(e, setting.default_cycle_months) })).filter((row: any) => query.status === 'all' || !query.status || (query.status === 'due' ? row.status === 'DUE' : row.status === 'UPCOMING' && row.months_remaining * 30 <= setting.upcoming_window_days)); }
  async employeeStatus(id: number) { const [employee, setting] = await Promise.all([this.prisma.employee_profiles.findUnique({ where: { id } }), this.settings()]); if (!employee) throw new NotFoundException('Employee not found'); return this.status(employee, setting.default_cycle_months); }
  ...
  async preview(dto: SalaryIncrementApplyDto, user: IJwtStaffPayload) { const [setting, employees] = await Promise.all([this.settings(), this.prisma.employee_profiles.findMany({ where: { id: { in: [...new Set(dto.employee_ids)] } } })]); return dto.employee_ids.map(id => { const e = employees.find(x => x.id === id); if (!e) return { employee_id: id, error: 'Employee not found' }; if (user.campusId != null && e.campus_id !== user.campusId) return { employee_id: id, error: 'Employee is outside your campus' }; if (e.employment_status !== 'ACTIVE') return { employee_id: id, error: 'Only active employees can receive an increment' }; if (!e.monthly_pay) return { employee_id: id, error: 'Monthly pay is not set' }; const next = this.newPay(e.monthly_pay, dto); return { employee_id: id, employee_name: e.full_name, previous_pay: Number(e.monthly_pay), new_pay: Number(next), annual_pay_before: Number(e.monthly_pay.mul(12)), annual_pay_after: Number(next.mul(12)), increment_amount: Number(next.minus(e.monthly_pay)), ...this.status(e, setting.default_cycle_months) }; }); }
  async apply(dto: SalaryIncrementApplyDto, user: IJwtStaffPayload) { const preview = await this.preview(dto, user); const batch = dto.employee_ids.length > 1 ? randomUUID() : null; const effective = dateOnly(dto.effective_from); const successes: any[] = [], failures = preview.filter((x: any) => x.error); for (const item of preview.filter((x: any) => !x.error)) { try { await this.prisma.$transaction(async tx => { const e = await tx.employee_profiles.findUniqueOrThrow({ where: { id: item.employee_id }, include: { employee_class_section_assignments: true } }); const old = e.monthly_pay!; const next = this.newPay(old, dto); const updated = await tx.employee_profiles.update({ where: { id: e.id }, data: { monthly_pay: next, last_increment_at: effective } }); const snapshot: EmployeeProgressionSnapshot = { campusId: updated.campus_id, segmentId: updated.segment_id, departmentId: updated.department_id, staffCategoryId: updated.staff_category_id, reportingManagerId: updated.reporting_manager_id, jobTitle: updated.job_title, employmentType: updated.employment_type, employmentStatus: updated.employment_status, monthlyPay: next, payrollEnabled: updated.payroll_enabled, classSections: e.employee_class_section_assignments.map(a => ({ class_id: a.class_id, section_id: a.section_id })) }; await this.progression.recordProgressionChange(tx, { ...snapshot, employeeId: e.id, changeType: 'SALARY_INCREMENT', changedBy: user.username ?? user.sub ?? null, notes: dto.notes, at: effective }); await tx.salary_increments.create({ data: { employee_id: e.id, mode: dto.mode, percentage: dto.mode === 'PERCENTAGE' ? dto.percentage : null, fixed_amount: dto.mode === 'FIXED_AMOUNT' ? dto.fixed_amount : null, previous_pay: old, new_pay: next, cycle_months_used: e.increment_cycle_months ?? (await this.settings()).default_cycle_months, effective_from: effective, applied_by: user.username ?? user.sub ?? null, notes: dto.notes ?? null, bulk_batch_id: batch } }); }); successes.push(item); } catch (err: any) { failures.push({ employee_id: item.employee_id, error: err.message ?? 'Could not apply increment' }); } } return { bulk_batch_id: batch, successes, failures }; }
}
