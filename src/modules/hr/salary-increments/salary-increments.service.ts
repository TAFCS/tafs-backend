...
export class SalaryIncrementsService {
  ...
  async due(query: DueSalaryIncrementsQueryDto, user: IJwtStaffPayload) { const setting = await this.settings(); const where: Prisma.employee_profilesWhereInput = { employment_status: 'ACTIVE', ...(user.campusId != null ? { campus_id: user.campusId } : query.campus_id ? { campus_id: query.campus_id } : {}), ...(query.department_id ? { department_id: query.department_id } : {}), ...(query.segment_id ? { segment_id: query.segment_id } : {}), ...(query.class_id ? { employee_class_section_assignments: { some: { class_id: query.class_id } } } : {}), ...(query.search ? { OR: [{ full_name: { contains: query.search, mode: 'insensitive' } }, { employee_code: { contains: query.search, mode: 'insensitive' } }] } : {}) };
    ...
    return rows.map(e => ({ employee_id: e.id, name: e.full_name, employee_code: e.employee_code, monthly_pay: e.monthly_pay ? Number(e.monthly_pay) : null, join_date: isoDate(e.join_date), last_increment_at: isoDate(e.last_increment_at), employment_status: e.employment_status, campus: e.campuses?.campus_name ?? null, department: e.departments?.name ?? null, segment: e.segments?.name ?? null, ...this.status(e, setting.default_cycle_months, setting.upcoming_window_days) })).filter((row: any) => { if (query.status === 'all' || !query.status) return true; if (query.status === 'due') return row.status === 'DUE'; return row.status === 'UPCOMING'; }); }
