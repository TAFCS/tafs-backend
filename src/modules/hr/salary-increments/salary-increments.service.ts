...
export class SalaryIncrementsService {
  constructor(private readonly prisma: PrismaService, private readonly progression: EmployeeProgressionService, private readonly scope: ScopeService) {}
  // Missing and out-of-scope both 404 — a 403 would confirm a real employee
  // exists at a campus outside the caller's scope.
  private async assertEmployee(id: number, user?: IJwtStaffPayload) {
    const employee = await this.prisma.employee_profiles.findUnique({ where: { id }, select: { id: true, campus_id: true, segment_id: true, department_id: true, staff_category_id: true } });
    if (!employee || (user && !this.scope.canSeeEmployee(user, employee))) throw new NotFoundException(`Employee ${id} not found`);
  }
  ...
  async due(query: DueSalaryIncrementsQueryDto, user: IJwtStaffPayload) { const setting = await this.settings(); const filters: Prisma.employee_profilesWhereInput = { employment_status: 'ACTIVE', ...((() => { const ids = this.scope.campusIdsFor(user, query.campus_ids); return ids ? { campus_id: { in: ids } } : {}; })()), ...(query.department_ids?.length ? { department_id: { in: query.department_ids } } : {}), ...(query.segment_ids?.length ? { segment_id: { in: query.segment_ids } } : {}), ...(query.staff_category_ids?.length ? { staff_category_id: { in: query.staff_category_ids } } : {}), ...(query.employment_types?.length ? { employment_type: { in: query.employment_types } } : {}), ...(query.search ? { OR: [{ full_name: { contains: query.search, mode: 'insensitive' } }, { employee_code: { contains: query.search, mode: 'insensitive' } }] } : {}) };
    ...
    // department at their campus, bypassing their own department scope).
    const universal = this.scope.whereForEmployees(user);
    const where: Prisma.employee_profilesWhereInput = Object.keys(universal).length > 0 ? { AND: [filters, universal] } : filters;
  ...
  async employeeStatus(id: number, user?: IJwtStaffPayload) { await this.assertEmployee(id, user); const [employee, setting] = await Promise.all([this.prisma.employee_profiles.findUnique({ where: { id } }), this.settings()]); if (!employee) throw new NotFoundException('Employee not found'); return this.status(employee, setting.default_cycle_months, setting.upcoming_window_days); }
  ...
  async analytics(user: IJwtStaffPayload) {
    ...
    const where: Prisma.salary_incrementsWhereInput = employeeProfileClauses.length > 0
      ? { employee_profiles: { AND: employeeProfileClauses } }
      : {};
  ...
  async updateEmployeeCycle(id: number, cycle: number | null, user?: IJwtStaffPayload) { await this.assertEmployee(id, user); if (cycle != null && (!Number.isInteger(cycle) || cycle < 1 || cycle > 120)) throw new BadRequestException('Increment cycle must be a whole number from 1 to 120 months.'); return this.prisma.employee_profiles.update({ where: { id }, data: { increment_cycle_months: cycle } }); }
