...
@Injectable()
export class ScholarshipPresetsService {
    constructor(
    ...
    async findAll(onlyActive = false) {
        return this.prisma.scholarship_presets.findMany({
    ...
    async findOne(id: number) {
        const preset = await this.prisma.scholarship_presets.findUnique({ where: { id } });
        if (!preset) throw new NotFoundException(`Scholarship preset #${id} not found`);
        return preset;
    }

    async create(dto: CreateScholarshipPresetDto, changedBy: string) {
        const created = await this.prisma.scholarship_presets.create({
            data: {
                title: dto.title,
        ...
        void this.auditLogs.log({
            entity_type: 'SCHOLARSHIP_PRESET',
    ...
    async update(id: number, dto: UpdateScholarshipPresetDto, changedBy: string) {
        const existing = await this.findOne(id);
        const updated = await this.prisma.scholarship_presets.update({
            where: { id },
            data: {
    ...
    async remove(id: number, changedBy: string) {
        ...
        const deleted = await this.prisma.scholarship_presets.update({
            where: { id },
