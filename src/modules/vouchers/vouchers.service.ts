...
export class VouchersService {
    ...
    async create(
        ...
        const voucher = await this.prisma.$transaction(async (tx) => {
            ...
            const generatorName = await this.resolveGeneratedByName(changedBy);
    ...
    /** Resolve the logged-in staff member's full name for the "Generated By" stamp on a voucher PDF. */
    async resolveGeneratedByName(userIdOrUsername?: string): Promise<string | undefined> {
        if (!userIdOrUsername || userIdOrUsername === 'system') return undefined;
        ...
        return byUsername?.full_name || undefined;
    }
    ...
    private async ensureVoucherGenerationMeta(
        voucherId: number,
        voucher: { generated_by_name?: string | null; generated_at?: Date | null },
        generatedByName: string | undefined,
        voucherData: { generatedByName?: string; generatedAt?: Date | string },
    ) {
